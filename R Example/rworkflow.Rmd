---
title: "Condatis connectivity analysis to plan resilient habitat networks - Condatis training in R - Workflow"
...
# Plot the habitat with 5 cover categories
plot(eghab,col=cividis(5,direction=-1),main="wetland habitat cover")
```
...
For the remaining plots/maps in this document, the code that produces them is hidden in the HTML version, but it can be seen by opening the rworkflow.Rmd file.
...
eghab = rast("data/wetland_east.tif")
direction = rast("data/east_W_E.tif")
plot(eghab,col=cividis(5,direction=-1), legend = FALSE, main="East-West Source and Target")
plot(direction, add=TRUE,col=c("cyan","purple"))
```
...
matches = TRUE
  if(!identical(myres,res(direction))){
    matches = FALSE
    warning(paste(
      "habitat has different resolution from st in direction",
      dirnow))
  }
  if(!identical(unitcheck,linearUnits(direction))){
    matches = FALSE
    warning(paste(
      "habitat has different distance unit from st in direction",
      dirnow))
  }
  if(matches){message(paste("st matches habitat in direction",dirnow))}

#assign a new object where this source-target raster is labelled 
#(as above, useful when running loops with multiple directions) 
...
We now use the `makehabitatpoints`and `stargetpt` to convert our rasters into data frames. The habitat data frame (here named `Wetland_East_hab`) contains the columns required to run the Condatis analysis (i.e. x,y and cover), and obtain our output maps (xm and ym). Similarly, the `stnow` list contains, the data frames for the source and targets.

```{r happt, eval=FALSE}

# Name habitat according to variable 'landname' and convert raster to points
assign(paste0(landname,"_hab"),
       makehabitatpoints(eghab,inm=identical(unitcheck,1)))

# Get source and target points
stnow = stargetpt(get(paste0(landname,"_",dirnow)),
                  inm=identical(unitcheck,1)) 
...
Now we can run the Condatis analysis. It can take some time, depending on the spec of your computer.
...
```{r test, echo=TRUE, message=TRUE, eval=FALSE}
...
if(jobok){
  ...
  # save raster
   writeRaster(progress, 
               paste0('outputs/', paste(landname, dirnow, "progress",sep="_"),'.tif'), 
               datatype = "FLT8S",
               overwrite = TRUE) 
   ...
   # save raster
   writeRaster(flow, 
               paste0('outputs/', paste(landname, dirnow, "flow",sep="_"),'.tif'), 
               datatype = "FLT8S",
               overwrite = TRUE) 
...
You can explore these rasters in your preferred GIS software, as well as plotting them in R, as below: 

```{r plotflowprogress, echo=FALSE}
library(terra)
landname = 'Wetland_East'  
dirnow = 'W_E'
#this reads the rasters from a different folder, just so that the Condatis code
#needn't be run to produce plots. Note that chunks above are not evaluated by default
progress = rast('QGIS_project/Wetland_East_W_E_progress.tif')
flow = rast('QGIS_project/Wetland_East_W_E_flow.tif')
par(mfrow = c(1, 2))
plot(progress,main = 'Progress', col=viridis(15,direction=-1))
plot(direction, add=TRUE,col=c("cyan","purple"),legend=F)
plot(sqrt(flow) , main = "sqrt of Flow",col=magma(12,direction=-1,end=0.9))
plot(direction, add=TRUE,col=c("cyan","purple"),legend=F)
```


You might notice that where the landscape shows discontinuities/jumps in progress is where the most serious bottlenecks tend to be (compare the above plot with the search areas below). Also, it is clear that the flow is concentrated around the easiest route from the source to the target.
...
```{r botlines, eval=FALSE}
  ...
  # save shapefile
  st_write(botlines, 
           paste0('outputs/', paste(landname, dirnow, "botlines",sep="_"),'.shp'),
           append = FALSE)
...
st_write(areas, 
         paste0('outputs/',paste(landname, dirnow, "areas",sep="_"),'.shp'),
         append=FALSE)
...
# Load data (again from files, so can be run independently of above chunks)
areas = st_read("QGIS_project/Wetland_East_W_E_areas.shp", quiet = TRUE)
eghab = rast("data/wetland_east.tif")
...
# Plot
plot(eghab,col=cividis(5,direction=-1), legend = FALSE)
...
Now you can explore this shapefile 'Wetland_East_W_E_areas.shp' in your preferred GIS software. You could overlay the bottleneck lines and notice how the 'areas' make it simpler to focus on the most impactful bottlenecks.

## Other directions - optional extra

If you wish, you can repeat the analyses above using other source-target directions. In the data folder, you will find the rasters for the Northwest-Southeast (NW_SE) and Southeast-East(SE_E) sources and targets.
The code in this part is somewhat trickier than the code above. The trickiness doesn't matter if you simply 'play' what we have written and see what happens. But it matters if you wanted to adapt the code for your own example landscape. 
...
```{r rename, echo=TRUE, message=TRUE}
landname = 'Wetland_East'  
dirnow = 'NW_SE'
```

These are the directions available in this exercise:

```{r STothers, echo=TRUE}
library(terra)

eghab = rast("data/wetland_east.tif")
direction = rast("data/east_W_E.tif")
direction2 = rast("data/east_NW_SE.tif")
direction3 = rast("data/east_SW_E.tif")

plot(eghab,col=cividis(5,direction=-1), legend = FALSE)
plot(direction, add=TRUE, legend = FALSE)
plot(direction2, add=TRUE, col =c('red','blue'), legend = FALSE)
plot(direction3, add=TRUE, col =c('orange','darkgreen'), legend = FALSE)
```

We have not provided you with code to compile these three directions into a loop and run them all. You would have to [Convert rasters to points] for these new directions and [Run Condatis] analyses with them. We'd only recommend doing this if you are practised in using R for loops.

## Multidirectional search areas

It is possible to combine the bottlenecks of multiple directions by appending the shapefiles of the [Bottleneck lines] (named *botlines*) for the multiple directions and apply the `zones_by_score_range` function (as described in [Search areas]). We have not provided code for this, but we have saved the end result in the QGIS_project folder. Here is a way to plot the 'search areas' considering the three directions of movement for this wetland landscape.

```{r allareas, echo=FALSE, message=FALSE, warning=FALSE}
library(sf)
library(terra)

# Load data
areas = st_read("QGIS_project/Wetland_East_combined_bottleneck_areas.shp", quiet = TRUE)
eghab = rast("data/wetland_east.tif")
...
# Plot
plot(eghab,col=cividis(5,direction=-1), legend = FALSE, main = "Search areas from three directions combined")
plot(areas, col = cols[areas$category], add = TRUE)
legend("topright", legend = names(cols), fill = cols, title = "Category", xpd = TRUE, bty = "n")
